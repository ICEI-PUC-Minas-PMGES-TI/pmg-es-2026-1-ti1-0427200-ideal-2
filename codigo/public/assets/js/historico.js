...
function calcularEstatisticas(recargas) {
  ...
  const totalKwh = concluidas.reduce(
    (soma, r) => soma + (r.energiaKwh || 0),
    0
  );

  const totalCusto = concluidas.reduce(
    (soma, r) => soma + (r.custoTotal || 0),
    0
  );
  ...
  try {
    switch (comando) {
      ...
      case "listar-status": {
        ...
        if (!status) {
          throw new Error(
            "Uso correto: node planejador.js listar-status <status>"
          );
        }
        ...
        if (recargas.length === 0) {
          console.log(`Nenhuma recarga com status '${status}' encontrada.`);
        } else {
          recargas.forEach((r) => console.log(formatarRecarga(r)));
        }
      ...
      case "listar-veiculo": {
        ...
        if (!veiculoId) {
          throw new Error(
            "Uso correto: node planejador.js listar-veiculo <veiculoId>"
          );
        }
        ...
        if (recargas.length === 0) {
          console.log(
            `Nenhuma recarga para o veículo #${veiculoId} encontrada.`
          );
        } else {
          recargas.forEach((r) => console.log(formatarRecarga(r)));
        }
      ...
      case "agendar": {
        if (args.length < 6) {
          throw new Error(
            "Uso correto: node planejador.js agendar <veiculoId> <postoId> <data> <horaInicio> <horaFim> <autonomiaInicialKm>"
          );
        }
      ...
      case "concluir": {
        if (args.length < 4) {
          throw new Error(
            "Uso correto: node planejador.js concluir <id> <energiaKwh> <custoTotal> <autonomiaFinalKm>"
          );
        }
      ...
      case "cancelar": {
        ...
        const atualizada = await cancelarRecarga(
          id,
          motivoPartes.join(" ")
        );
