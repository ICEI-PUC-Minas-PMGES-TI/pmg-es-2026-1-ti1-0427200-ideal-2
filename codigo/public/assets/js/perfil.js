...
btnEditar.addEventListener("click", ativarModoEdicao);
...
formPerfil.addEventListener("submit", (evento) => {
    ...
    dadosPendentes = { nome: novoNome, email: novoEmail, senha: novaSenha };
...
function abrirModalVerificacao(emailDestino) {
    ...
    console.log(`[Simulação] Código enviado para ${emailDestino}: ${CODIGO_VALIDO}`);
...
inputCodigo.addEventListener("keyup", (evento) => {
    if (evento.key === "Enter") btnConfirmarCodigo.click();
});
...
async function salvarAlteracoes() {
    try {
        ...
        if (!resposta.ok) throw new Error("Falha ao atualizar no servidor.");
